    ...
    if (isset($_POST['add_doctor'])) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target_dir . $filename)) {
                $image = 'uploads/doctors/' . $filename;
            }
        }

        $sql = "INSERT INTO doctors (name, specialty, experience, image, bio_link)
                VALUES ('$name', '$specialty', '$experience', '$image', '$bio_link')";
        $_SESSION['msg'] = $conn->query($sql) ? "Doctor added successfully!" : "Error: " . $conn->error;
...
<head>
    ...
    <style>
        ...
        .form-row input { flex:1; padding:12px; border:1px solid #ddd; border-radius:6px; }
...
<div class="admin-container">
    ...
    <div class="main-content">
        ...
        <div style="background:white;padding:25px;border-radius:8px;box-shadow:0 5px 15px rgba(0,0,0,0.05);margin-bottom:30px;">
            <h3 style="margin-bottom:20px;border-bottom:2px solid #f1f1f1;padding-bottom:10px;">Add New Doctor</h3>
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    <div style="flex:1;">
                        <label style="display:block;margin-bottom:8px;font-size:0.9rem;color:#666;font-weight:500;">Doctor Photo</label>
                        <input type="file" name="image" accept="image/*" required style="border:1px solid #ddd;padding:8px;width:100%;">
                    </div>
                </div>
        ...
        <div style="background:white;padding:25px;border-radius:8px;box-shadow:0 5px 15px rgba(0,0,0,0.05);">
            <h3 style="margin-bottom:20px;">Existing Doctors</h3>
            <div style="overflow-x:auto;">
                <table class="admin-table">
                    <thead><tr><th style="width:50px;">ID</th><th style="width:80px;">Photo</th><th>Name</th><th>Specialty</th><th>Experience</th><th>Action</th></tr></thead>
                    <tbody>
                    <?php
                        ...
                        echo "<tr>
                            ...
                            <td>
                                <div style='display:flex;gap:5px;'>
                                    <a href='edit_doctor.php?id={$row['id']}' class='btn-action btn-edit'><i class='fas fa-edit'></i></a>
                                    <form method='POST' style='display:inline;' onsubmit='return confirm(\"Remove this doctor?\");'>
                                        <input type='hidden' name='id' value='{$row['id']}'>
                                        <button type='submit' name='delete_doctor' class='btn-action btn-delete'><i class='fas fa-trash'></i></button>
                                    </form>
                                </div>
                            </td>
                          </tr>";
