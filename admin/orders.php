...
<div class="admin-container">
    ...
    <div class="main-content">
        ...
        <div style="background:white;padding:20px;border-radius:8px;box-shadow:0 5px 15px rgba(0,0,0,0.05);">
            <?php
            $sql    = "SELECT orders.id, users.username, users.email, users.phone,
                              orders.total_amount, orders.status, orders.created_at, orders.prescription_image
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $prescription_html = !empty($row['prescription_image'])
                        ? "<a href='/ease-meds/{$row['prescription_image']}' target='_blank'
                              style='display:inline-flex;align-items:center;gap:5px;color:#3498db;text-decoration:none;background:#ecf0f1;padding:5px 10px;border-radius:20px;font-size:0.9rem;'>
                              <i class='fas fa-file-prescription'></i> View</a>"
                    ...
                    echo "<tr>
                            ...
                            <td>
                                ...
                                <div style='font-size:0.8rem;color:#666;font-weight:500;'>" . (!empty($row['phone']) ? htmlspecialchars($row['phone']) : 'N/A') . "</div>
                            </td>
                            ...
                            <td>
                                <form method='POST' style='display:flex;flex-direction:column;gap:5px;'>
                                    ...
                                    <div style='display:flex;gap:5px;'>
                                        <select name='status' style='padding:5px;border-radius:4px;font-size:0.9rem;'>$options_html</select>
                                        <button type='submit' name='update_status' class='btn'
                                            style='padding:5px 8px;font-size:0.8rem;background:var(--primary-color);color:white;border:none;border-radius:4px;cursor:pointer;'>
                                            <i class='fas fa-check'></i>
                                        </button>
                                    </div>
                                </form>
                            </td>
                            <td style='text-align:center;'>$prescription_html</td>
                            <td>
                                <a href='/ease-meds/track_order.php?order_id=" . $row['id'] . "' target='_blank'
                                   style='color:#2d3436;font-size:0.9rem;text-decoration:underline;'>Details</a>
                            </td>
                          </tr>";
                }
